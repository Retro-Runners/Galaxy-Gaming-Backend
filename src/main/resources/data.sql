INSERT INTO products (quantity, price, description, image, name, type, console) VALUES (
   3,
   32.99,
   'Racing Game',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27333/18396/Game-GameCube-NFSU2-2__11534.1.jpg',
   'Need for Speed Underground 2',
   'Game',
   'Game Cube'
),
(
   1,
   79.99,
   'Party Game',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27314/18376/Game-GameCube-MP5-2__66365.1.jpg',
   'Mario Party 5',
   'Game',
   'Game Cube'
),
(
   5,
   19.99,
   'Spider-Man',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27388/18460/Game-GameCube-Spiderman-2__62355.1.jpg',
   'Spider-Man',
   'Game',
   'Game Cube'
),
(
   5,
   89.99,
   'Luigis Mansion',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/19007/23004/Luigis_Mansion_box__40032.1.jpg',
   'Luigis Mansion',
   'Game',
   'Game Cube'
),
(
   4,
   99.99,
   'Legend of Zelda The Wind Waker',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/19018/11287/Complete%20Gamecube-73-2__02566.1.jpg',
   'Legend of Zelda The Wind Waker',
   'Game',
   'Game Cube'
),
(
   0,
   148.99,
   'Paper Mario',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/19058/11325/Complete%20Gamecube-Paper-2__29177.1.jpg',
   'Paper Mario',
   'Game',
   'Game Cube'
),
(
   4,
   64.99,
   'Animal Crossing',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27201/18237/Game-Gamecube-AnCross-2__86134.1.jpg',
   'Animal Crossing',
   'Game',
   'Game Cube'
),
(
   1,
   39.99,
   'Tony Hawks UNDERGROUND 2',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/19003/11271/Complete%20Gamecube-58-2__42385.1.jpg',
   'Tony Hawks UNDERGROUND 2',
   'Game',
   'Game Cube'
),
(
   4,
   19.99,
   'Harry Potter Prisoner of Azkaban',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27270/18330/Game-Gamecube-HarryPPOA-2__90499.1.jpg',
   'Harry Potter Prisoner of Azkaban',
   'Game',
   'Game Cube'
),
(
   5,
   19.99,
   'Dragon Ball Z Budokai',
   'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/19046/11313/Complete%20Gamecube-DragonBallZ-2__00652.1.jpg',
   'Dragon Ball Z Budokai',
   'Game',
   'Game Cube'
),
(
    4,
    17.99,
    'Lego Star Wars',
    'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27287/18348/Game-Gamecube-legosw-2__42495.1.jpg',
    'Lego Star Wars',
    'Game',
    'Game Cube'
),
(
     3,
     12.99,
     'The Sims',
     'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/27373/18444/Game-Gamecube-Sims-2__31150.1.jpg',
     'The Sims',
     'Game',
     'Game Cube'
),
(
       5,
       9.99,
       'AV Cords for Game Cube',
       'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/16382/9186/Acc_Gamecube-av-2__71242.1.jpg',
       'AV Cords for Game Cube',
       'Accessory',
       'Game Cube'
),
(
    6,
    178.99,
    'Gaming Console',
    'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/31687/23595/Black_gamecube_console__44570.1.jpg',
    'Game Cube Console',
    'Console',
    'Game Cube'
),
(
    3,
    48.99,
    'Original Controller',
    'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/175x175/products/16379/9183/Acc_Gamecube-1-2__22395.1.jpg',
    'Original Controller Black',
    'Accessory',
    'Game Cube'
),

(
    5,
    34.99,
    'Adventure',
    'https://m.media-amazon.com/images/I/618K8K3FZGL.jpg',
    'Pokemon Ruby',
    'GBA',
    'Game'
),

(
    4,
    34.99,
    'Adventure',
    'https://m.media-amazon.com/images/I/61RDD36KWPL.jpg',
    'Pokemon Sapphire',
    'GBA',
    'Game'
),

(
    4,
    34.99,
    'Adventure',
    'https://m.media-amazon.com/images/I/510BNCVM1NL.jpg',
    'Pokemon Emerald',
    'GBA',
    'Game'
),

(
    5,
    44.99,
    'Action',
    'https://m.media-amazon.com/images/I/619-6scn5wL.jpg',
    'Mario Kart: Super Circuit',
    'GBA',
    'Game'
),

(
    3,
    11.99,
    'Action',
    'https://i.ebayimg.com/images/g/x3EAAOSwk5FUylj4/s-l300.jpg',
    'Pac-Man Collection',
    'GBA',
    'Game'
),

(
    5,
    39.99,
    'Platform',
    'https://m.media-amazon.com/images/I/610qecq5-TL.jpg',
    'Donkey Kong Country',
    'GBA',
    'Game'
),

(
    4,
    16.99,
    'Adventure',
    'https://m.media-amazon.com/images/I/618HZN4PK8L.jpg',
    'Finding Nemo',
    'GBA',
    'Game'
),

(
    5,
    28.99,
    'Platform',
    'https://i.ebayimg.com/images/g/CnUAAMXQiM5Q9djE/s-l300.jpg',
    'Sonic Advance',
    'GBA',
    'Game'
),

(
    5,
    29.99,
    'Platform',
    'https://m.media-amazon.com/images/I/61v+MQXtaRL.jpg',
    'Super Mario Advance',
    'GBA',
    'Game'
),

(
   5,
   109.99,
   'Clear GameBoy Advance console', 'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/320w/products/31074/35247/GBA-Clear__24645.1668463414.jpg?c=2',
   'GameBoy Advance',
   'GBA',
   'Console'
),
(
    3,
    23.99,
    'Magnifies your screen for improved viewing',
    'https://i.ebayimg.com/images/g/GGQAAOSwMaphB~fF/s-l500.jpg',
    'Screen Magnifier',
    'GBA',
    'Accessory'
),

(
    2,
    39.99,
    'Enables up to 5 players to use the multiplayer mode wirelessly',
    'https://cdn.shopify.com/s/files/1/0248/9538/9795/products/bum_1024x.png?v=1631167889',
    'Wireless Adapter',
    'GBA',
    'Accessory'
),
 (
     4,
     79.99,
     '2D Sifi Shooter',
     'https://upload.wikimedia.org/wikipedia/en/e/e4/Smetroidbox.jpg',
     'Super Metroid',
     'Game',
     'SNES'
 ),
 (
     4,
     34.99,
     'Platformer',
     'https://images.emulatorgames.net/super-nintendo/super-mario-world.jpg',
     'Super Mario World',
     'Game',
     'SNES'
 ),
 (
     5,
     57.99,
     'Racer',
     'https://upload.wikimedia.org/wikipedia/en/3/38/Supermariokart_box.JPG',
     'Super Mario Kart',
     'Game',
     'SNES'
 ),

 (
     4,
     28.47,
     'Input device for snes',
     'https://images.lukiegames.com/t_300e2/assets/images/snes/snes_orig_ctl_p_yvn1uo.jpg',
     'Super Nintendo Controller',
     'Accessory',
     'SNES'
 ),
 (
     5,
     29.99,
     'Used to play game boy games on big screen',
     'https://cdn11.bigcommerce.com/s-ymgqt/images/stencil/320w/products/16357/40869/sgameboyplyr-snes__46818.1668448674.jpg?c=2',
     'SNES SUPER GAMEBOY PLAYER',
     'Accessory',
     'SNES'
 ),
 (
     5,
     335.76,
     'SNES console',
     'https://videogameperfection.com/wp-content/uploads/2017/08/Nintendo-SNES.jpg',
     'Super Nintendo Entertainment System',
     'Console',
     'SNES'
 ),


   (
       5,
       36.96,
       'Platform',
   'https://upload.wikimedia.org/wikipedia/en/thumb/0/03/Super_Mario_Bros._box.png/220px-Super_Mario_Bros._box.png',
       'Super Mario Bros',
       'NES',
       'Game'
   ),

   (
       4,
       77.76,
       'Adventure',
   'https://upload.wikimedia.org/wikipedia/en/thumb/4/41/Legend_of_zelda_cover_%28with_cartridge%29_gold.png/220px-Legend_of_zelda_cover_%28with_cartridge%29_gold.png',
       'The Legend of Zelda',
       'NES',
       'Game'
   ),

   (
       4,
       42.95,
       'Platform',
     'https://upload.wikimedia.org/wikipedia/en/thumb/0/00/Super_Mario_Bros_2.jpg/220px-Super_Mario_Bros_2.jpg',
       'Super Mario Bros 2',
       'NES',
       'Game'
   ),

   (
       5,
       48.68,
       'Platform',
   'https://upload.wikimedia.org/wikipedia/en/thumb/a/a5/Super_Mario_Bros._3_coverart.png/220px-Super_Mario_Bros._3_coverart.png',
       'Super Mario Bros 3',
       'NES',
       'Game'
   ),

   (
       3,
       35.34,
       'Action',
   'https://upload.wikimedia.org/wikipedia/en/thumb/5/5d/Metroid_boxart.jpg/220px-Metroid_boxart.jpg',
       'Metroid',
       'NES',
       'Game'
   ),

   (
       5,
       53.95,
       'Fighting',
   'https://upload.wikimedia.org/wikipedia/en/thumb/5/51/Castlevania_1_cover.png/220px-Castlevania_1_cover.png',
       'Castlevania',
       'NES',
       'Game'
   ),

   (
       4,
       119.98,
       'Platform',
   'https://upload.wikimedia.org/wikipedia/en/thumb/9/9b/Rockman_1987.jpg/220px-Rockman_1987.jpg',
       'Mega Man',
       'NES',
       'Game'
   ),

   (
       5,
       36.99,
       'Adventure',
   'https://m.media-amazon.com/images/M/MV5BOGUxZjcyMjktYmU2Yy00ZTEyLTg3NjctMjRlODg0Nzg5OTlmXkEyXkFqcGdeQXVyNjExODE1MDc@._V1_.jpg',
       'Metal Gear',
       'NES',
       'Game'
   ),

   (
       3,
       19.95,
       'Strategy',
   'https://upload.wikimedia.org/wikipedia/en/thumb/7/7d/Tetris_NES_cover_art.jpg/220px-Tetris_NES_cover_art.jpg',
       'Tetris',
       'NES',
       'Game'
   ),

   (
       5,
       37.99,
       'First Person Shooter',
       'https://upload.wikimedia.org/wikipedia/en/1/14/DuckHuntBox.jpg',
       'Duck Hunt',
       'NES',
       'Game'
   ),
   (
         5,
         213.99,
         'Gray Nintendo Entertainment System console',
         'https://cdn.arstechnica.net/wp-content/uploads/2013/07/NES-Console-Set.jpg',
         'Nintendo Entertainment System',
         'NES',
         'Console'
      ),
      (
         4,
         54.99,
         'Arcade style controller', 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Nintendo-NES-Advantage-Controller.jpg/220px-Nintendo-NES-Advantage-Controller.jpg',
         'NES Advantage',
         'NES',
         'Accessory'
      ),
      (
          3,
          28.99,
          'Main controller for NES',
      'https://upload.wikimedia.org/wikipedia/commons/thumb/b/b5/Nintendo-Entertainment-System-NES-Controller-FL.jpg/120px-Nintendo-Entertainment-System-NES-Controller-FL.jpg',
          'NES Controller',
          'NES',
          'Accessory'
      ),

      (
          2,
          23.99,
          'Allows player to point at the screen and “shoot” at in-game targets',
      'https://i.etsystatic.com/8948780/r/il/a9dec4/1918404357/il_fullxfull.1918404357_pt7l.jpg',
          'NES Zapper',
          'NES',
          'Accessory'
      ),

      (
          5,
          175,
          'Robot that can substitute for a second player on NES',
      'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6c/NES-ROB.jpg/220px-NES-ROB.jpg',
          'Robotic Operating Buddy (ROB)',
          'NES',
          'Accessory'
      )
;

INSERT INTO users (id, email, pass_word, first_name, last_name, address) VALUES (
    1,
    'testuser@gmail.com',
    'password',
    'Test',
    'User',
    ''
);
